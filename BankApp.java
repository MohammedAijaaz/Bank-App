...
class Account implements Serializable									// User Account class
{
	int acc_no;
...
class MyFrame extends Frame												// The Main Frame to which all panels are added
{
	...
	{	
		...
		if(f.exists())
		{
			...
			try
			{
				ObjectInputStream ois=new ObjectInputStream(new FileInputStream("AccountDB.txt"));
		...
		addWindowListener(new WindowAdapter(){
			public void windowClosing(WindowEvent we)
			{
				try
				{
					ObjectOutputStream oos=new ObjectOutputStream(new FileOutputStream("AccountDB.txt"));
					oos.writeObject(hm);
					oos.close();
				}
				catch(Exception e)
...
class AdminLogin extends Panel implements ActionListener
{
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		{
			if(tf.getText().equals("IamTheAdmin"))
			{
...
class Register extends Panel implements ActionListener					//	Panel_3: Register Page
{
	...
	Checkbox m;
		...
		public void actionPerformed(ActionEvent ae)
		{
			Account nAcc=new Account(myFrame.acno);
			if(ae.getActionCommand().equals("<-(Back)"))
			{
				myFrame.remove(this);
				HomePage p=new HomePage(myFrame);
...
class UserPage extends Panel implements ActionListener					//	Panel_4: User Page
{
	...
	UserPage(Account acc,MyFrame myFrame)
	{
		...
		lbal=new Label("$ "+Double.toString(acc.bal));
	...
	public void actionPerformed(ActionEvent ae)
	{
		try{
			...
			{
				acc.bal+=Double.parseDouble(dep_amount.getText());
			...
			{
				if(acc.bal-Double.parseDouble(with_amount.getText())<1000)
				{
				...
				{
					acc.bal-=Double.parseDouble(with_amount.getText());
					lbal.setText("$ "+Double.toString(acc.bal));
